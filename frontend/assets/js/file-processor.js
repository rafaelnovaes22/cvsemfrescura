...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Atualizar o botão de análise com base nos créditos do usuário
    function updateAnalyzeButton(credits) {
        const analyzeBtn = document.getElementById('analyzeBtn');
        if (!analyzeBtn) return;
        
        if (credits > 0) {
            analyzeBtn.textContent = `Analisar Currículo (${credits} disponível${credits > 1 ? 's' : ''})`;
            analyzeBtn.disabled = false;
        } else {
            analyzeBtn.textContent = 'Comprar Análises';
    ...
    function setupAnalyzeButton() {
        ...
        // Verificar créditos do usuário atual
        const user = window.auth && window.auth.getUser();
        if (user && user.credits !== undefined) {
            updateAnalyzeButton(user.credits);
        } else {
            // Comportamento padrão para usuários sem informação de créditos
